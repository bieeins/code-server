...
export class LinkProvider implements DocumentLinkProvider {
    ...
    public provideDocumentLinks(document: TextDocument, token: CancellationToken): ProviderResult<DocumentLink[]> {
        ...
        let documentLinks = [];
        let index = 0;
        let reg = /((\:\:)|(->))(((get|post|put|patch|delete|options|any|resource|apiResource|singleton|apiSingleton)\()|(match\(\s?\[(['"][a-zA-Z]+['"],?\s?)+\],))\s?['"](\/?([a-zA-Z\d_\-.#@\/]|(\{[a-zA-Z\d_\-]+(\}|\?\})))*\/?)?(\?[a-zA-Z\d_]+\=[\s\S]*)?['"],\s?((['"][a-zA-Z\d_\\]+(@[a-zA-Z\d_]+|::[a-zA-Z\d_]+)?['"])|(\[(([a-zA-Z\d_\\]+::class)|(['"][a-zA-Z\d_\\]+['"])),\s?['"][a-zA-Z\d_]+['"]\])|([a-zA-Z\d_\\]+::class))\s?\)/;
