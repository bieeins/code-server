 ...
 * @param document
 */
export function getFilePath(text: string, document: TextDocument, composerAutoLoadPSR: Array<string>) {

    let workspaceFolder = workspace.getWorkspaceFolder(document.uri).uri.fsPath + pathCtrl;

    // replace the http controllers namespace to empty string
    let controllerFileName = text.replace(pathNamespace, '').replace('\\\\', '') + '.php';
